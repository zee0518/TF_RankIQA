...
def process_command_args():
    ...
    parser.add_argument('--train_list',type=str,default= os.path.abspath('..')+'/data/ft_live_train.txt',help='train data list for read image.')
    parser.add_argument('--test_list', type=str, default= os.path.abspath('..')+'/data/ft_live_test.txt', help='test data list for read image.')
...
def train(args):
    ...
    with tf.Session(graph=graph) as sess:
        ...
        for step, (images, targets) in enumerate(train_loader,start_step):
            ...
            loss_,y_hat_, _ = sess.run([reg_loss,y_hat,optimizer], feed_dict={imgs: images,scores: targets,lr: base_lr,
